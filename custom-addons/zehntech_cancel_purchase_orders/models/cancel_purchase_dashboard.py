...
class CancelPurchaseDashboard(models.Model):
    ...
    purchase_kanban_bar_chart = fields.Text(
        string="Purchase Bar Chart",
        compute="_compute_bar_json"
    )

    purchase_bar_html = fields.Html(
        string="Colored Bar Graph",
        compute="_compute_purchase_bar_html"
    )
    ...
    # Computed field to show the selected custom date range (if any)
    ...
    def _compute_purchase_bar_html(self):
        for record in self:
            ...
            <div style="text-align:center; font-size:12px; margin-top:2px;">
                {_("RFQ")}: {record.draft_count} |
                {_("Purchase Order")}: {record.confirmed_count} |
                {_("Cancelled")}: {record.cancelled_count}
            </div>
            """
    ...
    def action_open_draft_records(self):
        ...
        return {
            'name': _('Quotation Purchase Orders'),
            'type': 'ir.actions.act_window',
            'res_model': 'purchase.order',
            'view_mode': 'list',
            'views': [(self.env.ref('zehntech_cancel_purchase_orders.view_purchase_order_tree_no_bulk_actions').id, 'list')],
            'domain': domain,
