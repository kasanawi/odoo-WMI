    ...
    def action_cancel_and_delete(self):
        ...
        for order in self:
            ...
            associated_data = {
                ...
                'order_line': [(0, 0, {'product_id': line.product_id.id, 'product_qty': line.product_qty})
    ...
    def _cancel_associated_records(self,order):
        ...
        for picking in self.picking_ids:
            if picking.state != 'done':
                ...
                order.message_post(body=_("Receipt %s is already done and cannot be canceled." % picking.name))
    ...
    def _reset_associated_records(self,order):
        ...
        for picking in order.picking_ids:
            if picking.state != 'done':
                ...
                order.message_post(body=_("Receipt %s is already done and cannot be reset to draft." % picking.name))
        ...
        for bill in order.invoice_ids:
            if bill.state == 'cancel':
                ...
                order.message_post(body=_("Bill %s is posted/paid and was not reset to draft." % bill.name))
